...
def read_worker(q_in,q_out,q_status,cat_dict, attr_validation, val2cat, cat_num, attr_num, attr_dict):
    cat_count = np.array([[0,0] for i in range(cat_num+1)],dtype='int' )
    failed = 0
    ...
    while True:
        ...
        q_out.put( (data['img_id'], data['path'], bbox, cat_vector, attr_vector) )
    q_status.put(( cat_count, failed) )

def write_worker(q_out, f, proc):
    cnt = proc
    idx = 0
    stime = time.time()
    while True:
        idx += 1
        if idx%10000 == 0:
            print idx, 'done, qsize:', q_out.qsize(), time.time()-stime, 's'
            stime = time.time()
        data = q_out.get()
        if data == None:
            break
...
def make_txt(cat_num,attr_num,target):
    ...
    while sel_id < maxid:
        ...
        sel_id += offset
        print len(datas), len(datas)/proc, 'per process'
        # shuffle(datas)

        with open(target,'a') as f:
            q_in = [multiprocessing.Queue(1000) for _ in range(proc)]
            q_out = multiprocessing.Queue(2000)
            q_status = multiprocessing.Queue(1000)

            process_list = [multiprocessing.Process(target=read_worker, \
                args=(q_in[i],q_out,q_status, cat_dict, attr_validation,val2cat,cat_num,attr_num, attr_dict )) \
                for i in range(proc)]
...
def visualize_txt(label_path):
    cat_path = './label_v1/category.txt'
    cat_dict,val2cat = load_cat(cat_path)
    attr_path = './label_v1/attribute.json'
...
def balance_category(in_name, out_name, max_num):
    ...
    categories_count = [max_num for i in range(49)]
    ###### add special rules here
    categories_count[37] = 10000 #门
    categories_count[24] = 10000 #窗
...
if __name__ == "__main__":
    make_txt(cat_num = 45, attr_num = 154, target = '/data/data/jiahuan/label/furniture/furinture_0205/furniture_shop.txt')
    # visualize_txt('/data/data/jiahuan/label/furniture/furinture_0130/furniture_shop.txt')
    ...
    # balance_category('/data/data/jiahuan/label/furniture/furinture_0130/furniture_shop.txt',
    #     '/data/data/jiahuan/label/furniture/furinture_0130/furniture_shop_balance.txt',
    #     50000)
    # make_train_val('/data/data/jiahuan/label/furniture/furinture_0130/furniture_shop_balance.txt',500000,50000)
